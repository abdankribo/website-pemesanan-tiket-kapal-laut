...
export default async function MyTicketsPage() {
  ...
  function TicketCard({ticket,historyCard=false}:{ticket:(typeof tickets)[number];historyCard?:boolean}) {
    ...
    const badgeClass=verified?"bg-[#27F5F5]/20 text-primary":cancelled||expired?"bg-slate-100 text-slate-500":"bg-[#d6ed00]/32 text-primary";
    return (
      <article className={`interactive-card aesthetic-card rounded-3xl p-5 ${verified&&historyCard?"ring-2 ring-[#27F5F5]/30":""}`}>
        {verified&&historyCard&&<div className="mb-4 flex items-center gap-2 rounded-2xl bg-[#27F5F5]/7 px-4 py-3 text-sm font-bold text-primary"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-[#00a8b0]">✓</span>Tiket sudah digunakan dan terverifikasi</div>}
    ...
    <main className="min-h-screen px-4 py-10 pb-28">
      <div className="mx-auto max-w-3xl">
        <header className="flex items-center justify-between gap-4">
          <div><p className="text-xs font-black uppercase tracking-[.3em] text-[#0079ad]">Account</p><h1 className="mt-1 text-3xl font-black text-primary">Tiket Saya</h1><p className="mt-1 text-sm text-slate-500">Boarding pass aktif dan riwayat perjalanan.</p></div>
        ...
        {tickets.length===0 ? <div className="aesthetic-card mt-8 rounded-[2rem] p-10 text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#27F5F5]/10 text-primary"><span className="material-symbols-outlined text-2xl">confirmation_number</span></span><p className="mt-5 text-lg font-black text-primary">Belum ada tiket</p><p className="mt-2 text-sm text-slate-500">Pesan tiket kapal untuk melihat boarding pass Anda di sini.</p><Link href="/booking" className="primary-btn mt-5 inline-flex rounded-xl px-5 py-3 text-sm font-bold">Pesan Tiket</Link></div> : <div className="mt-8 space-y-8">
          <section><div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-black uppercase tracking-wider text-primary">Tiket Aktif</h2><span className="rounded-full bg-[#d6ed00]/32 px-2.5 py-1 text-[10px] font-black text-primary">{active.length}</span></div>{active.length?<div className="space-y-4">{active.map(ticket=><TicketCard key={ticket.ticketId} ticket={ticket}/>)}</div>:<div className="aesthetic-card rounded-2xl p-5 text-sm text-slate-500">Tidak ada tiket aktif.</div>}</section>
