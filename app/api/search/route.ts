...
export async function GET(request:Request){
 ...
 if(!origin||!destination||origin===destination||!/^\d{4}-\d{2}-\d{2}$/.test(departureDate)||Number.isNaN(d.getTime())||d<today)return NextResponse.redirect(new URL("/?error=Data%20pencarian%20tidak%20valid",request.url));
