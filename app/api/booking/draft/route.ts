...
export async function POST(request:Request){
 ...
 const draft=await prisma.bookingDraft.create({data:{userId:user.id,payload:{origin:String(b.origin||"ujung"),destination:String(b.destination||"kamal"),departureDate:String(b.departureDate),passengerName:name,passengerNik:nik,passengerPhone:phone,vehicle,vehiclePlate:vehicle==="passenger"?null:plate,serviceType:String(b.serviceType||"passenger"),basePrice:prices[vehicle as keyof typeof prices]},expiresAt:new Date(Date.now()+30*60*1000)}});
