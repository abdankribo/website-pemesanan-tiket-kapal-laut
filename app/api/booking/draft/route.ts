...
export async function POST(request:Request){
 ...
 if(name.length>100||!name||!/^[0-9]{16}$/.test(nik)||!/^08\d{8,11}$/.test(phone)||!(vehicle in prices)||!validDate(String(b.departureDate))||!["passenger","motor","car"].includes(serviceType)||((vehicle==="motor"||vehicle==="car")&&!/^[A-Z]{1,2} ?\d{4} ?[A-Z]+$/.test(plate))) return NextResponse.json({error:"Data booking tidak valid"},{status:400});
