...
export async function POST(request:Request){
  ...
  let form: FormData;
  try { form = await request.formData(); } catch { return NextResponse.redirect(new URL("/account?error=invalid", request.url)); }
  ...
  if(name.length<1||name.length>100||email.length>254||!/^\S+@\S+\.\S+$/.test(email)||!/^(08)\d{8,11}$/.test(phone)||!/^[0-9]{16}$/.test(nik)||!birthDate) return NextResponse.redirect(new URL("/account?error=invalid",request.url));
